...
import { Bell, Shield, Phone, Mail, User as UserIcon, LogOut, LogIn, Check, Lock } from 'lucide-react';
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 relative overflow-hidden">
              ...
              <div className="space-y-4 opacity-100 transition-opacity">
                  <div className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition">
                      <div>
                          <p className="font-medium text-gray-900">Likes & Reactions</p>
                          <p className="text-xs text-gray-500">When someone likes your posts</p>
                      </div>
                      <button 
                        onClick={() => handleToggle('likes')}
                        disabled={!user}
                        className={`w-12 h-6 rounded-full transition-colors relative ${prefs.likes ? 'bg-primary' : 'bg-gray-300'}`}
                      >
                          <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-transform ${prefs.likes ? 'left-7' : 'left-1'}`} />
                      </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition">
                      ...
                      <button 
                        ...
                        disabled={!user}
                        className={`w-12 h-6 rounded-full transition-colors relative ${prefs.comments ? 'bg-primary' : 'bg-gray-300'}`}
                      >
                          <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-transform ${prefs.comments ? 'left-7' : 'left-1'}`} />
                      </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition">
                      <div>
                          ...
                          <p className="text-xs text-gray-500">When you are mentioned in a post</p>
                      </div>
                      <button 
                        onClick={() => handleToggle('mentions')}
                        disabled={!user}
                        className={`w-12 h-6 rounded-full transition-colors relative ${prefs.mentions ? 'bg-primary' : 'bg-gray-300'}`}
                      >
                          <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-transform ${prefs.mentions ? 'left-7' : 'left-1'}`} />
                      </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition">
                      ...
                      <button 
                        ...
                        disabled={!user}
                        className={`w-12 h-6 rounded-full transition-colors relative ${prefs.system ? 'bg-primary' : 'bg-gray-300'}`}
                      >
                          <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-transform ${prefs.system ? 'left-7' : 'left-1'}`} />
                      </button>
                  </div>
              </div>
          </div>
